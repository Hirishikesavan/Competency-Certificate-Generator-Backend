...
namespace CompetencyCertificate.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        {
            Console.WriteLine($"Request received for department: {departmentId}"); // or use ILogger
            ...
            try
            {
                Console.WriteLine("try block"); // or use ILogger
                var normalized = departmentId.Trim().ToLower();
                Console.WriteLine($"normalized: {normalized}"); // or use ILogger
                var count = _context.Employee.Count(e =>
                    e.DepartmentName != null &&
                    e.DepartmentName.Trim().ToLower() == normalized
                );
                Console.WriteLine($"count: {count}"); // or use ILogger
